...
public sealed partial class Player
{
	...
	public void RequestBuyMiscItem( string prefabPath )
	{
		...
		{
			Notices.SendNotice( Network.Owner, "block", Color.Red, $"Vous possédez déjà {TipJar.MaxOwnedPerPlayer} boîte(s) à pourboires.", 3 );
			return;
		}
		...
		{
			Notices.SendNotice( Network.Owner, "block", Color.Red, $"Vous possédez déjà {Lawboard.MaxOwnedPerPlayer} tableau(x) de lois.", 3 );
			return;
		}
		...
		Notices.SendNotice( Network.Owner, "block", Color.Red, "Impossible de placer cet objet pour le moment.", 3 );
	}
}
