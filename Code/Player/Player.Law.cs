...
public sealed partial class Player
{
	...
	public void RequestSetWantedStatus( Guid playerId, string reason, bool wanted )
	{
		...
		if ( wanted )
		{
			...
			{
				Notices.SendNotice( Network.Owner, "block", Color.Red, "Indiquez une raison pour le signalement.", 3 );
				return;
			}
	...
	public void RequestArrestPlayer( Guid playerId )
	{
		...
		{
			Notices.SendNotice( Network.Owner, "block", Color.Red, "Approchez-vous pour arrêter ce joueur.", 3 );
			return;
		}
	...
	public void RequestReleaseArrestPlayer( Guid playerId )
	{
		...
		if ( WorldPosition.Distance( target.WorldPosition ) > ArrestDistance )
		{
			Notices.SendNotice( Network.Owner, "block", Color.Red, "Approchez-vous pour libérer ce joueur.", 3 );
			return;
		}
		...
		{
			Notices.SendNotice( actorConnection, "check_circle", Color.Green, $"{target.DisplayName} a été libéré(e).", 3 );
		}
	}
	...
	public void SetWanted( string reason, Player actor )
	{
		...
		Scene.Get<Chat>()?.AddSystemText( $"{DisplayName} est recherché(e) : {PlayerData.WantedReason}", "🚨" );
	}
	...
	public void ClearWanted( Player actor )
	{
		...
		if ( actor?.Network.Owner is { } actorConnection )
		{
			Notices.SendNotice( actorConnection, "check_circle", Color.Green, $"Signalement supprimé pour {DisplayName}.", 3 );
		}
	}
	...
	public void BeginArrest( Player officer )
	{
		...
		if ( TryFindArrestLocation( out var arrestLocation ) )
		{
			...
			{
				Notices.SendNotice( officerSuccessConnection, "check_circle", Color.Green, $"{DisplayName} a été déplacé(e) en cellule.", 3 );
			}
		}
		else if ( officer?.Network.Owner is { } officerNoticeConnection )
		{
			Notices.SendNotice( officerNoticeConnection, "block", Color.Orange, "Aucune cellule trouvée. Le joueur a été arrêté sur place.", 4 );
		}
		...
		{
			Notices.SendNotice( officerConnection, "gavel", Color.Green, $"{DisplayName} a été arrêté(e).", 3 );
		}
