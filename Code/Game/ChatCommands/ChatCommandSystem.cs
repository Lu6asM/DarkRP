...
public static class ChatCommandSystem
{
	...
	[
		new( "advert", "/advert <message>", "Diffuser une annonce publique.", AdvertCommand, aliases: ["ad"] ),
		new( "ooc", "/ooc <message>", "Parler hors personnage.", OocCommand, aliases: ["//"] ),
		new( "me", "/me <action>", "Décrire une action roleplay.", MeCommand ),
		new( "pm", "/pm <joueur> <message>", "Envoyer un message privé.", PrivateMessageCommand, aliases: ["msg", "tell", "w"] ),
		new( "dropmoney", "/dropmoney <montant>", "Jeter de l'argent devant vous.", DropMoneyCommand, aliases: ["dropcash"] ),
		new( "name", "/name <nom rp>", "Changer votre nom roleplay.", NameCommand, aliases: ["rpname", "nick"] ),
		new( "kick", "/kick <joueur> [raison]", "Expulser un joueur.", KickCommand, ChatCommandAccess.Admin, accessText: "admin" ),
		new( "ban", "/ban <joueur|steamid> [raison]", "Bannir un joueur.", BanCommand, ChatCommandAccess.SuperAdmin, accessText: "superadmin" ),
		new( "unban", "/unban <steamid>", "Retirer un ban par SteamID.", UnbanCommand, ChatCommandAccess.SuperAdmin, accessText: "superadmin" ),
		new( "setadmin", "/setadmin <joueur> <none|admin|superadmin>", "Modifier le rôle admin d'un joueur.", SetAdminCommand, ChatCommandAccess.SuperAdmin, accessText: "superadmin" ),
		new( "givemoney", "/givemoney <joueur> <montant>", "Donner de l'argent à un joueur.", GiveMoneyCommand, ChatCommandAccess.Admin, accessText: "admin" ),
		new( "setmoney", "/setmoney <joueur> <montant>", "Définir l'argent d'un joueur.", SetMoneyCommand, ChatCommandAccess.Admin, accessText: "admin" )
	];
	...
	static void PrivateMessageCommand( ChatCommandContext context )
	{
		...
		context.Chat?.AddSystemTextTo( context.Connection, $"MP à {target.DisplayName} : {message}", "PM" );
	...
	static void BanCommand( ChatCommandContext context )
	{
		...
		{
			context.Reply( "Joueur introuvable. Utilisez un SteamID pour bannir un joueur hors ligne.", "!" );
			return;
		}
	...
	static void SetAdminCommand( ChatCommandContext context )
	{
		...
		{
			context.Reply( "Utilisation : /setadmin <joueur> <none|admin|superadmin>", "!" );
			return;
		}
		...
		{
			context.Reply( "Vous ne pouvez pas modifier le rôle de l'hôte.", "!" );
			return;
		}

		AdminSystem.Current?.SetRole( connection.SteamId, role, connection.DisplayName );
		Notices.SendNotice( context.Connection, "security", Color.Green, $"Rôle de {target.DisplayName} défini sur {role}.", 3 );
	}
	...
	static void GiveMoneyCommand( ChatCommandContext context )
	{
		...
		Notices.SendNotice( context.Connection, "$", Color.Green, $"Vous avez donné {amount:n0}$ à {target.DisplayName}.", 3 );
	}
