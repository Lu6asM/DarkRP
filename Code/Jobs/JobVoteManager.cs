...
public sealed class JobVoteManager : Component
{
	...
	public bool TryStartVote( Player candidate, JobDefinition definition, out string reason )
	{
		...
		Scene.Get<Chat>()?.AddSystemText( $"{CandidateName} souhaite devenir {JobTitle}. Ouvrez le menu contextuel avec C pour déverrouiller votre souris et voter.", "how_to_vote" );
		return true;
	}
	...
	void FinishVote()
	{
		...
		{
			Scene.Get<Chat>()?.AddSystemText( $"Le vote de {candidateName} a été approuvé, mais le candidat n'est plus disponible.", "how_to_vote" );
			return;
		}
		...
		if ( !JobManager.CanJoin( candidate, definition, out var reason ) )
		{
			...
			Scene.Get<Chat>()?.AddSystemText( $"Le vote de {candidateName} a été approuvé, mais le métier {jobTitle} n'est plus disponible.", "how_to_vote" );
			return;
		}

		candidate.ApplyVotedJobDefinition( definition );
		Scene.Get<Chat>()?.AddSystemText( $"Le vote de {candidateName} pour {jobTitle} a été approuvé ({yesVotes} oui / {noVotes} non).", "how_to_vote" );
	}
